...
- Use Codex or another LLM to analyze structured metric results.
...
- Arbitrary PromQL execution by Codex or any LLM.
- n8n workflow orchestration in Phase 1.

## Codex's Role

Codex is the AI analysis layer. Codex should:
...
Codex must not directly access Prometheus, execute arbitrary PromQL, or perform infrastructure changes.
...
## Collaboration Rules Between Codex and Codex

- Codex provides metric reasoning and response guidance.
...
- Let Codex or any LLM execute arbitrary PromQL directly.
...
## How Codex Should Answer Metric Questions
...
On-demand Teams chat uses a two-agent Codex flow implemented in `POST /teams/chat`:
...
## How Codex Should Use Backend Tools

Codex must use controlled backend tools only. It should not generate open-ended PromQL.
